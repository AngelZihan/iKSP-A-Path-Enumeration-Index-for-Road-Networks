...
int Graph::eKSPNew(int ID1, int ID2, int k, vector<int>& kResults, vector<vector<int> >& vkPath) 
{
	...
	/*			for(int i = vmPathNodePos[pID][eNodeID2]; i < (int)vvPathCandidate[pID].size(); i++)  
				{
					...
					else
					{
						bFixedLoop = true; 
						cout << "Loop" << endl; 
						cout << vvPathCandidate[pID][i] << "\t" << vvPathCandidate[pID].size() << endl;
						for(int j = 0; j < (int)vvPathCandidate[pID].size(); j++)     
							cout << vvPathCandidate[pID][j] << "\t";
						cout << endl;
...
//				cout << "New Dist:" << dist << endl << endl;	 
			...
			//	if(mE.size() != vPath.size())
			//		cout << mE.size() << "\t" << vPath.size() << endl; 
